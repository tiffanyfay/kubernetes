...
type DockerConfigProvider interface {
	...
	// Implementations can be blocking - e.g. metadata server unavailable.
	Provide(repoToPull string) DockerConfig
	// LazyProvide(repoToPull string) gets called after URL matches have been performed, so the
	// location used as the key in DockerConfig would be redundant.
	LazyProvide(repoToPull string) *DockerConfigEntry
}

//LazyProvide returns an Lazy AuthConfig
func LazyProvide(creds LazyAuthConfiguration, repoToPull string) AuthConfig {
	if creds.Provider != nil {
		entry := *creds.Provider.LazyProvide(repoToPull)
...
func (d *defaultDockerConfigProvider) Provide(repoToPull string) DockerConfig {
...
func (d *defaultDockerConfigProvider) LazyProvide(repoToPull string) *DockerConfigEntry {
	return nil
}
...
// LazyProvide implements dockerConfigProvider. Should never be called.
func (d *CachingDockerConfigProvider) LazyProvide(repoToPull string) *DockerConfigEntry {
	return nil
}

// Provide implements dockerConfigProvider
func (d *CachingDockerConfigProvider) Provide(repoToPull string) DockerConfig {
	d.mu.Lock()
	defer d.mu.Unlock()
	...
	d.cacheDockerConfig = d.Provider.Provide(repoToPull)
